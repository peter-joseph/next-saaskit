...
export function MyToastRegion() {
  return (
    ...
    <ToastRegion
      queue={queue}
      className="fixed bottom-4 right-4 flex flex-col-reverse gap-2 rounded-lg outline-none focus-visible:outline-solid focus-visible:outline-2 focus-visible:outline-accent-600 focus-visible:outline-offset-2">
      {({toast}) => (
        <MyToast toast={toast}>
  ...
  return (
    <Toast
      ...
      className={composeTailwindRenderProps(
        props.className,
        "flex items-center gap-4 bg-accent-600 px-4 py-3 rounded-lg outline-none forced-colors:outline focus-visible:outline-solid focus-visible:outline-2 focus-visible:outline-accent-600 focus-visible:outline-offset-2 [view-transition-class:toast] font-sans w-[230px]"
      )}
    />
  );
}
